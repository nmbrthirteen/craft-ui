...
The `%`-stops in `@keyframes t-input-shake` are cumulative leg durations as a fraction of the total. The default leg pattern is **A, A, B, B**: the two big-swing legs (right peak → left peak) take `--shake-dur-a` each, and the two recovery legs (left peak → overshoot → rest) take `--shake-dur-b` each:
