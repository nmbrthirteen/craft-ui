...
Tweening a container's width or height when its layout state changes (compact to expanded card, collapsing panel, list row toggling extra detail). Pure CSS, with no JS required beyond the class toggle that drives the size change.
...
None. This is pure CSS. Toggle the documented HTML attributes or class names from whatever already drives state in your app.
